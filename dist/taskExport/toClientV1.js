...
function normalizeNativeLanguage(lang) {
    const s = String(lang ?? "zh");
    if (s.toLowerCase() === "zh-cn" || s === "zh_CN")
        return "zh";
    return s;
}
...
function statusToClientCode(status) {
    switch (status) {
        case "draft":
            return 0;
        case "pending_review":
...
function hashTaskModelId(taskId) {
    let h = 0;
    for (let i = 0; i < taskId.length; i++)
        h = (h * 31 + taskId.charCodeAt(i)) >>> 0;
    return (h % 999_999) + 1;
}
const PHASE_TITLE_EN = {
...
function findPhaseByType(phases, phaseType) {
    for (const ph of phases) {
        const p = asRecord(ph);
        if (p && String(p.type) === phaseType)
            return p;
    }
    return null;
}
function findStepByType(phase, stepType) {
    const steps = Array.isArray(phase.steps) ? phase.steps : [];
    for (const st of steps) {
        const s = asRecord(st);
        if (s && String(s.type) === stepType)
            return s;
    }
    return null;
}
function mergeGuidance(phaseG, stepG) {
...
function mergeTwoGuidances(a, b) {
    if (!a || (!a.purpose && !a.description))
        return b ?? { purpose: "", description: "" };
    if (!b || (!b.purpose && !b.description))
        return a;
    return {
...
function trFlat(flat, key) {
    return flat[key] ?? "";
}
function tltsToClient(tlts) {
    const empty = {
        words: [],
        phrases: [],
        sentences: [],
...
function assetsToClient(assets) {
    ...
    if (images) {
        for (const [id, a] of Object.entries(images)) {
            ...
            if (img?.prompt != null)
                row.prompt = String(img.prompt);
            if (img?.url != null)
                row.url = String(img.url);
            if (img?.base64 != null)
                row.base64 = String(img.base64);
            out.push(row);
        }
    }
    ...
    if (audios) {
        for (const [id, a] of Object.entries(audios)) {
            ...
            if (au?.prompt != null)
                row.prompt = String(au.prompt);
            if (au?.url != null)
                row.url = String(au.url);
            if (au?.base64 != null)
                row.base64 = String(au.base64);
            out.push(row);
        }
    }
    return out;
}
function feedbackPrinciplesToClient(fp) {
    if (!Array.isArray(fp))
        return [{ mode: "immediate", strategies: [] }];
    if (fp.length === 0)
        return [{ mode: "immediate", strategies: [] }];
    const first = fp[0];
 ...
 */
function mapQuestionType(internal) {
    if (internal === "audio_text")
        return "audio_text";
    return "text_choice";
}
function mapQuestion(q, tr) {
    ...
    if (stem.audioAssetId != null)
        out.audioAssetId = stem.audioAssetId;
    if (stem.imageAssetId != null)
        out.imageAssetId = stem.imageAssetId;
    return out;
}
function flattenWordPhraseSentenceQuestions(bucket, tr) {
    const b = asRecord(bucket);
    if (!b)
        return [];
    const out = [];
...
function buildQuestionsForSubtask(sub, dialogues) {
    ...
    for (const dist of distractors) {
        ...
        const stemText = promptTurn?.text != null ? String(promptTurn.text) : correctText || `Turn ${turnIndex}`;
        const optionsRaw = Array.isArray(d.options) ? d.options : [];
        const opts = [];
        let idCounter = 1;
        const pushOpt = (id, text) => {
        ...
        const correctId = opts.find((o) => o.text === correctText)?.id ?? opts[0]?.id;
        questions.push({
            type: "text_choice",
            stem: stemText,
            translation: "",
...
function phraseClozeToQuestions(clozeMap, tr) {
...
function sentenceToSortQuestion(sentence, tr) {
...
function buildQuestionsForRoleplay(roleplay, dialogues, tr) {
    const dialogueId = String(roleplay.dialogueId ?? "");
    const dlg = dialogues.find((d) => String(d.id) === dialogueId);
    if (!dlg || !Array.isArray(dlg.turns))
        return [];
    const turns = dlg.turns.map((t) => asRecord(t) ?? {});
    const learnerRole = String((Array.isArray(roleplay.allowedRoles) ? roleplay.allowedRoles[0] : null) ?? "user");
    ...
    for (const h of hints) {
        ...
        for (let j = 0; j < turns.length; j++) {
            if (j === idx)
                continue;
            const t = turns[j];
            if (String(t.role ?? "") !== learnerRole)
                continue;
            const tx = String(t.text ?? "");
            if (tx && tx !== correctText)
                otherTexts.push(tx);
        }
...
function clientPhaseShell(clientType, step, phaseRecord, labels) {
...
function buildClientPhases(data, taskModel, tr) {
    const phases = Array.isArray(data.phases) ? data.phases : [];
    const dialogues = Array.isArray(taskModel.dialogues)
        ? taskModel.dialogues.map((d) => asRecord(d) ?? {})
        : [];
    const out = [];
...
function pickImageAssetId(data, taskModel) {
    ...
    if (entry?.thumbnail != null && String(entry.thumbnail))
    ...
    for (const ph of phases) {
        ...
        for (const st of steps) {
            const s = asRecord(st);
            if (s?.thumbnail != null && String(s.thumbnail))
                return String(s.thumbnail);
        }
    }
    return undefined;
}
 ...
 */
export function toClientTaskExportV1(input) {
    ...
    const behavioral = Array.isArray(taskModel.behavioralChain)
        ? taskModel.behavioralChain.slice(0, 20)
        : [];
    const taskModelOut = {
    ...
    if (root.learningGoals && Array.isArray(root.learningGoals) && root.learningGoals.length === 0) {
